...
!function (def) {
	...
	Flow.prototype._getIterator = function (item, returnResult) {
		...
		var BaseOne = {
			getArguments: function () {
				if ('waterfall' == mode) {
					...
					var lastReturnValue = returnResult.pop();
					if (lastReturnValue) {
			...
			getNext: function () {
				throw new Error('Abstract crap to implement');
			}
		};

		if (typeof item == 'function') {
			BaseOne.getNext = function () {
			...
			var stack = item._stack;
			var index = 0;
			var current = null;
			// push crap to return
			BaseOne.getNext = function () {
				if (!current) {
					current = item._getIterator(stack[index++], returnResult);
				}
				...
				if (current) {
					...
					if (next) {
						...
						// has no more items, in current, taking next one
						current = null;
						return this.getNext();
					}
				} 
			};
			// pop crap from return
		}
	...
	Flow.prototype.run = function (cb, scope) {
		...
		var getReturn = function () {
			var zork = returnArray.map(function (returnObject) {
		...
		var iterator = this._getIterator(this, returnArray);
		...
		var ae = new AwesomeExecuter({
			next : function () {
				...
				var values = getReturn();
				cb.call(context, null, values);
				return null;
			},

			addToReturn: function (returnObject) {
				// skipping first value, since it's an null from the error
				var zork = returnObject.slice(1);
				returnArray.push(zork);
			},
